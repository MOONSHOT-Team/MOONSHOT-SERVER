...
public class ImageController {
    ...
    @GetMapping("/image")
    public ResponseEntity<ApiResponse<PresignedUrlVO>> getPresignedUrl() {
        return ResponseEntity.status(HttpStatus.OK).body(ApiResponse.success(SuccessType.GET_PRESIGNED_URL_SUCCESS, s3Service.getUploadPreSignedUrl("test", "SMC")));
    }
    ...
    @PostMapping("/image")
    public ResponseEntity<ApiResponse<?>> notifyImageSaveSuccess(@RequestBody NotifyImageSaveSuccessRequestDto request) {
        s3Service.notifyImageSaveSuccess(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success(SuccessType.POST_NOTIFY_IMAGE_SAVE_SUCCESS));
    }
